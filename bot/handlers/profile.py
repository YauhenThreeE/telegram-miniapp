...
async def update_activity(message: Message, state: FSMContext) -> None:
    ...
    if value is None:
        ...
        return
    await save_field(message.from_user.id, "activity_level", value)
...
async def update_goal(message: Message, state: FSMContext) -> None:
    ...
    if value is None:
        ...
        return
    await save_field(message.from_user.id, "nutrition_goal", value)
    await finish_edit(message, state, lang)
...
async def update_numeric_field(message: Message, state: FSMContext, field_name: str) -> None:
    ...
    if value is None:
        ...
        return
    await save_field(message.from_user.id, field_name, value)
    await finish_edit(message, state, lang)


async def save_field(telegram_id: int, field_name: str, value) -> None:
    async with async_session_maker() as session:
